...
insert into User (pname, password, age, email) values ('test', 'weffewfw', 18, 'fjwelk@126.com');
delete from User where pid = 1;


DROP TABLE IF EXISTS `User`;
CREATE TABLE `User` (
  `pid` int NOT NULL COMMENT '用户ID' auto_increment,
  `pname` varchar(100) DEFAULT NULL COMMENT '用户名',
  `password` varchar(100) DEFAULT NULL COMMENT '密码',
  `age` int DEFAULT NULL COMMENT '令牌',
  `email` varchar(100) COMMENT '昵称',
  PRIMARY KEY (`pid`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

update user set pname='#{name}',password='#{password}', email='#{email}', age='#{age}' where pid = 1;
...
select new.cn_user_name,new.cn_notebook_name
from (select cn_user.cn_user_name, cn_notebook_name from cn_notebook, cn_user where cn_user.cn_user_id=cn_notebook.cn_user_id) new
where new.cn_user_name='ewoifuoi';


update cn_note set cn_note_type_id=3

delete from cn_note where cn_note_id='028dea02-19cb-4c50-b20a-569e86957913';
select * from cn_note where cn_note_id='019cd9e1-b629-4d8d-afd7-2aa9e2d6afe0';

select * from cn_note where cn_user_id='602c7c2f-137e-413f-883d-2d72268e31ac' and cn_note_status_id='recycled' ;

update cn_note set cn_note_status_id = 0 where cn_note_id='17b37b01-1f9a-48d3-aca9-74f5a14c7ae7';

delete from cn_notebook where cn_notebook_id='0037215c-09fe-4eaa-aeb5-25a340c6b39b';
...
update cn_note set cn_notebook_id = '' ,cn_note_status_id=null where cn_note_id='';

select * from cn_note where cn_user_id='b39894f3-04c8-43b0-a5ba-b4e4ec144536' and cn_note_status_id='recycled' ;
...
insert into sys_log (operation, create_time, method_name, param_name, ip) values (operation, create_time, method_name, param_name, ip);
